...
public class ElGamalBob
{
	private static boolean verifySignature(	BigInteger y, BigInteger g, BigInteger p, BigInteger a, BigInteger b, String message)
	{
		...
		BigInteger yap;
		BigInteger abp;
		BigInteger m = BigInteger.valueOf(message.hashCode());
		
		yap = y.modPow(a, p);
		abp = a.modPow(b, p);
		left = (yap.multiply(abp)).mod(p);
		
		right = g.modPow(m, p);
		
		System.out.println("yap="+yap.intValue());
		System.out.println("abp="+abp.intValue());
		System.out.println("left="+left.intValue());
		System.out.println("right="+right.intValue());
